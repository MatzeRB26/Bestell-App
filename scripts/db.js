let menuList = [
    {
        imgPath: "./assets/img/veggie-burger.jpg",
        title: "Veggie mushroom black burger",
        price: 16.90,
        description: "Mixed green salad, Tomatoes, Edamame, Mushrooms",
    },
{
        imgPath: "./assets/img/all-meat-burger.jpg",
        title: "All meat burger",
        price: 15.90,
        description: "Beef, Bacon, Dill pickles, Smoked cheese, Ketchup, BBQ souse",
    },
    {
        imgPath: "./assets/img/beef-red-burger.jpg",
        title: "Beef red burger",
        price: 14.90,
        description: "Beef, Cheese, Tomatoes, Lettuce, Onion",
    },
    {
        imgPath: "./assets/img/big-chicken-burger.jpg",
        title: "Big chicken burger",
        price: 15.90,
        description: "Chicken, Cheese, Tomatoes, Lettuce, Onion, Bell pepper",
    },
];

let menuListTwo = [
    {
        imgPath: "./assets/img/pizza-margherita.jpg",
        title: "Pizza Margherita",
        price: 11.90,
        description: "Tomato Sauce, Mozzarella",
    },
{
        imgPath: "./assets/img/pizza-chorizo.jpg",
        title: "Pizza Chorizo",
        price: 13.90,
        description: "Tomato slices, Mozzarella, Chorizo",
    },
    {
        imgPath: "./assets/img/pizza-funghi.jpg",
        title: "Funghi",
        price: 12.90,
        description: "Red Onion, Olives, Button Mushrooms, Mozzarella",
    },
    {
        imgPath: "./assets/img/pizza-quattro.jpg",
        title: "Quattro Formaggi with Chicken",
        price: 15.90,
        description: "Chicken Mozzarella, Gorgonzola, Fontina, Parmigiano Reggiano",
    },
];

let menuListThree = [
    {
        imgPath: "./assets/img/beef-salad.jpg",
        title: "Warm beef arugula salad",
        price: 16.90,
        description: "Beef, Arugula, Field salad, Greek feta, Cherry tomatoes, Sun-dried Tomatoes, Balsamic-vinegar dressing",
    },
{
        imgPath: "./assets/img/mini-salad.jpg",
        title: "Mini green salad",
        price: 7.90,
        description: "Green salad, Cucumber, Carrots, Parsley, Radishes",
    },
    {
        imgPath: "./assets/img/green-salad.jpg",
        title: "Green Salad with sea food",
        price: 16.90,
        description: "Mixed greens, Cherry tomatoes, Red onion, Mussels, Squid rings, Shrimp, Dijon mustard-lemon dressing with dill",
    },
    {
        imgPath: "./assets/img/vegan-salad.jpg",
        title: "Vegan green salad with tofu",
        price: 14.90,
        description: "Green salad, Cherry tomatoes, Cucumber, Baby spinach, Edamame. Radishes, Bittercress, Tofu, Peanuts",
    },
];

let basket = [];